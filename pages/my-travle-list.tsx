...
function MyTravleList() {
  ...
  const dDayCalculate = useCallback((e: any) => {
    ...
    if (startStatus > 0 && startStatus < 1) {
      ...
      result =
        '여행가기' +
        Math.ceil(moment.duration(moment(e.startDay).hours(0).diff(moment())).asDays()) +
        '일 전';
      ...
      <>
        ...
        >
          <Top.Root
            ...
            title={<Top.TitleParagraph typography="t3">{item.item.travelName}</Top.TitleParagraph>}
            subtitle1={
            ...
            subtitle2={
              <Top.SubtitleParagraph typography="t6" color={colors.blue600} fontWeight="medium">
                {
                  dDayCalculate({
