...
function PresetDetail() {
  ...
  const restaurantRecommend = useCallback(
    async ({ value, index, idx }: RecommendOptions): Promise<any> => {
      const timetable = presetDatas[params?.index];
      ...
      if (index === timetable[idx].length - 1) {
        ...
        if (
          timetable[idx][index - 1].name.includes('추천') &&
          timetable[idx][index + 1].name.includes('추천')
        )
          goCheck = false;
        else if (timetable[idx][index - 1].name.includes('추천'))
  ...
  const confirmRecommend = (autoRecommendFlag: boolean) =>
    bottomSheet.open({
      children: (
        <>
          <Text
