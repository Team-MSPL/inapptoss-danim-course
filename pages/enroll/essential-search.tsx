...
function EnrollEssentialSearch() {
  ...
  const { Place, essentialPlaces, accommodations } = useAppSelector((state) => state.travelSlice);
  ...
  const addPlace = (newHour, datas) => {
    const filteredPlaces = essentialPlaces.filter((place) => place?.day === params.idx + 1);
