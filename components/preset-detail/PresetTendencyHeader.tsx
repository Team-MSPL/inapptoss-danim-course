...
export function PresetTendencyHeader({ index, tendencyList, calculateTendency }: PresetTendencyHeaderProps) {
    if (!tendencyList[index]?.tendencyNameList || tendencyList[index].tendencyNameList.length < 2) return null;
    return (
        <ListRow
            left={
                <ListRow.Image
