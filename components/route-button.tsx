...
export const RouteButton = ({
  ...
  const { region, country, cityIndex } = useAppSelector((state) => state.travelSlice);
    ...
    if (region.length == 0) {
      ...
      const data = isDomestic
        ...
        : `해외/${countryEn}/${city.title.normalize('NFD')} ${cityEng.normalize(
            'NFD',
          )}${cityEng ? ' ' : ''}${regionName !== '전체' ? '!' + regionName : '!' + subTitle}`;
  ...
  const handleNext = () => {
    ...
    const nowStep = navigation.getState()?.routes?.at(-1)?.name.split('/enroll')[1];
    if (nowStep == '/region') {
      const shouldProceed = goNext();
      if (shouldProceed) {
        navigation.navigate((nowStep == '/distance' ? '' : '/enroll') + routeStack[nowStep]?.next);
      }
  ...
  return (
    <FixedBottomCTA.Double
      ...
      rightButton={
        <Button display="block" onPress={handleNext} disabled={disabled ?? false}>
          다음으로
        </Button>
      }
    />
