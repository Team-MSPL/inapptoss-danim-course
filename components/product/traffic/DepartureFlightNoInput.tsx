...
export default function DepartureFlightNoInput({
  ...
  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.departure_flightNo ?? "");
  ...
  useEffect(() => {
    setTrafficField(trafficType, "departure_flightNo", value);
    onValueChange?.(value);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [value, trafficType]);
