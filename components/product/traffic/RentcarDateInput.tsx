...
/**
 * RentcarDateInput: arrival Date와 동일 UI 패턴
 */
export default function RentcarDateInput({ trafficType, field, label, required = false, onValueChange }: Props) {
  ...
  useEffect(() => {
    if (year && month && day) {
      ...
      setTrafficField(trafficType, field, val);
      onValueChange?.(val);
    } else {
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      <Text typography="t6" color={colors.grey800} style={{ marginBottom: 6 }}>
        {label ?? (field === "s_date" ? "대여 시작일" : "대여 종료일")} {required ? <Text style={{ color: colors.red400 }}>*</Text> : null}
      </Text>
