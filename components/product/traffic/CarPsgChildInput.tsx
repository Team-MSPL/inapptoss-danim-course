...
export default function CarPsgChildInput({ trafficType, label = "어린이 수", placeholder = "0", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.carpsg_child ?? "");
  ...
  useEffect(() => {
    ...
    setTrafficField(trafficType, "carpsg_child", num === null || isNaN(num) ? "" : num);
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      ...
      <TextInput placeholder={placeholder} placeholderTextColor={colors.grey400} value={value} onChangeText={(t) => setValue(t.replace(/[^0-9]/g, ""))} style={styles.input} keyboardType="numeric" accessibilityLabel={`carpsg-child-${trafficType}`} />
    </View>
  );
}
