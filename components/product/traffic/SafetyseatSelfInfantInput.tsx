...
export default function SafetyseatSelfInfantInput({
  ...
  const stored = useBookingStore((s: any) => s.trafficArray?.find((it: any) => String(it?.traffic_type) === String(trafficType))?.safetyseat_self_infant ?? "");
  ...
  useEffect(() => {
    ...
    setTrafficField(trafficType, "safetyseat_self_infant", num === null || isNaN(num) ? "" : num);
