...
export default function ArrivalTimeInput({ trafficType, label = "도착 시간", placeholder = "HH:mm", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.arrival_time ?? "");
  ...
  useEffect(() => {
    setTrafficField(trafficType, "arrival_time", value);
    onValueChange?.(value);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [value, trafficType]);
