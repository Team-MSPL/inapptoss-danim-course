...
export default function ArrivalDateInput({ trafficType, label = "도착 날짜", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.arrival_date ?? "");
  ...
  useEffect(() => {
    if (year && month && day) {
      ...
      setTrafficField(trafficType, "arrival_date", val);
      onValueChange?.(val);
    } else {
      setTrafficField(trafficType, "arrival_date", "");
      onValueChange?.("");
    }
