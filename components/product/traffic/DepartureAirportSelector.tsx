...
export default function DepartureAirportSelector({
                                                   trafficType,
                                                   rawFields,
  ...
  const trafficSpec = Array.isArray(rawFields?.traffics)
    ...
    : null;
  const options: Option[] = trafficSpec?.departure_airport?.list_option ?? [];

  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.departure_airport ?? null);
  ...
  useEffect(() => {
    if (selectedCode !== null && selectedCode !== "") setTrafficField(trafficType, "departure_airport", String(selectedCode));
    else setTrafficField(trafficType, "departure_airport", "");
        ...
        <View style={styles.dropdown}>
          <ScrollView nestedScrollEnabled>
            {options.map((opt, idx) => {
              ...
              return (
                <TouchableOpacity key={String(code) + String(idx)} onPress={() => { setSelectedCode(String(code)); setOpen(false); }} style={[styles.optionRow, active ? styles.optionRowActive : undefined]}>
