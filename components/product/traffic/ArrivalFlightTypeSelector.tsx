...
export default function ArrivalFlightTypeSelector({
                                                    trafficType,
                                                    rawFields,
  ...
  const trafficSpec = Array.isArray(rawFields?.traffics)
    ...
    : null;
  const options: Option[] = trafficSpec?.arrival_flightType?.list_option ?? [];

  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.arrival_flightType ?? null);
  ...
  useEffect(() => {
    if (selectedId !== null && selectedId !== "") setTrafficField(trafficType, "arrival_flightType", String(selectedId));
    else setTrafficField(trafficType, "arrival_flightType", "");
