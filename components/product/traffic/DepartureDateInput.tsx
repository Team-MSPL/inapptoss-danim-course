...
export default function DepartureDateInput({ trafficType, label = "출발 날짜", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.departure_date ?? "");
  ...
  useEffect(() => {
    if (year && month && day) {
      ...
      setTrafficField(trafficType, "departure_date", val);
      onValueChange?.(val);
    } else {
      setTrafficField(trafficType, "departure_date", "");
      onValueChange?.("");
    }
