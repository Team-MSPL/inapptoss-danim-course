...
export default function SafetyseatSupInfantInput({ trafficType, label = "유아 보조석(대여) 수(인펀트)", placeholder = "0", required = false, onValueChange }: Props) {
  ...
  useEffect(() => {
    ...
    setTrafficField(trafficType, "safetyseat_sup_infant", num === null || isNaN(num) ? "" : num);
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      ...
      <TextInput placeholder={placeholder} placeholderTextColor={colors.grey400} value={value} onChangeText={(t) => setValue(t.replace(/[^0-9]/g, ""))} style={styles.input} keyboardType="numeric" accessibilityLabel={`safetyseat-sup-infant-${trafficType}`} />
    </View>
  );
}
