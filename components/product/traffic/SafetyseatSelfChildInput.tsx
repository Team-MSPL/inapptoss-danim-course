...
export default function SafetyseatSelfChildInput({ trafficType, label = "아동 카시트 개인 카시트 사용", placeholder = "0", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.safetyseat_self_child ?? "");
  ...
  useEffect(() => {
    ...
    setTrafficField(trafficType, "safetyseat_self_child", num === null || isNaN(num) ? "" : num);
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      ...
      <TextInput placeholder={placeholder} placeholderTextColor={colors.grey400} value={value} onChangeText={(t) => setValue(t.replace(/[^0-9]/g, ""))} style={styles.input} keyboardType="numeric" accessibilityLabel={`safetyseat-self-child-${trafficType}`} />
    </View>
  );
}
