...
export default function CarPsgInfantInput({ trafficType, label = "유아 수", placeholder = "0", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.trafficArray?.find(it => String(it?.traffic_type) === String(trafficType))?.carpsg_infant ?? "");
  ...
  useEffect(() => {
    ...
    setTrafficField(trafficType, "carpsg_infant", num === null || isNaN(num) ? "" : num);
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      ...
      <TextInput placeholder={placeholder} placeholderTextColor={colors.grey400} value={value} onChangeText={(t) => setValue(t.replace(/[^0-9]/g, ""))} style={styles.input} keyboardType="numeric" accessibilityLabel={`carpsg-infant-${trafficType}`} />
    </View>
  );
}
