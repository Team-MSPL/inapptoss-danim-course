...
type Props = {
  cusType?: string; // 기본 "send"로 사용
  required?: boolean;
  onValueChange?: (value: string) => void;
};

export default function CheckInDateInput({ cusType = "send", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.customMap?.[cusType]?.check_in_date ?? "");
  ...
  // 체크인/체크아웃은 미래 선택이므로 currentYear ~ currentYear + 5 범위 제공
  const years = useMemo(() => {
    const arr: number[] = [];
  ...
  useEffect(() => {
    if (year && month && day) {
      ...
      setCustomField(cusType, "check_in_date", val);
      onValueChange?.(val);
    } else {
      setCustomField(cusType, "check_in_date", "");
      onValueChange?.("");
    }
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      <Text typography="t6" color={colors.grey800} style={{ marginBottom: 6 }}>
        체크인 날짜 {required ? <Text style={{ color: colors.red400 }}>*</Text> : null}
      </Text>
