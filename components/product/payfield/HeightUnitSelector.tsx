...
type Props = {
  cusType: string;
  options?: Option[]; // rawFields.custom.height_unit.list_option
  label?: string;
  required?: boolean;
  onValueChange?: (v: string | null) => void;
};

export default function HeightUnitSelector({ cusType, options = [], label = "키 단위", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.customMap?.[cusType]?.height_unit ?? null);
  ...
  useEffect(() => {
    if (selectedId !== null) {
      setCustomField(cusType, "height_unit", selectedId);
      onValueChange?.(selectedId);
    } else {
      setCustomField(cusType, "height_unit", "");
      onValueChange?.(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedId, cusType]);
