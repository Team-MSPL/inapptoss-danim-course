...
type Props = {
  cusType: string;
  options?: Option[]; // rawFields.custom.shoe_type.list_option
  label?: string;
  required?: boolean;
  onValueChange?: (v: string | null) => void;
};

export default function ShoeTypeSelector({ cusType, options = [], label = "신발 종류", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.customMap?.[cusType]?.shoe_type ?? null);
  ...
  useEffect(() => {
    if (selectedId !== null) {
      setCustomField(cusType, "shoe_type", selectedId);
      onValueChange?.(selectedId);
    } else {
      setCustomField(cusType, "shoe_type", "");
      onValueChange?.(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedId, cusType]);
