...
type Props = {
  cusType: string;
  options?: Option[]; // rawFields.custom.shoe_unit.list_option
  label?: string;
  required?: boolean;
  onValueChange?: (v: string | null) => void;
};

export default function ShoeUnitSelector({ cusType, options = [], label = "신발 단위", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.customMap?.[cusType]?.shoe_unit ?? null);
  ...
  useEffect(() => {
    // 저장은 id 우선, 없으면 code, 없으면 빈 문자열
    if (selectedId !== null) {
      setCustomField(cusType, "shoe_unit", selectedId);
      onValueChange?.(selectedId);
    } else {
      setCustomField(cusType, "shoe_unit", "");
      onValueChange?.(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedId, cusType]);
