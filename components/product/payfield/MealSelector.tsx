...
type Props = {
  cusType: string;
  options?: Option[]; // rawFields.custom.meal.list_option
  label?: string;
  required?: boolean;
...
/**
 * MealSelector
 * - 화면에 meal_type_name을 보여주고, 선택 시 meal_type 값을 상태에 저장
 */
export default function MealSelector({ cusType, options = [], label = "식사 유형", required = false, onValueChange }: Props) {
  const stored = useBookingStore((s) => s.customMap?.[cusType]?.meal ?? null);
  ...
  useEffect(() => {
    if (selectedId !== null) {
      setCustomField(cusType, "meal", selectedId);
      onValueChange?.(selectedId);
    } else {
      setCustomField(cusType, "meal", "");
      onValueChange?.(null);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedId, cusType]);
